...
pub struct BrowserMainType<T: 'static> {
    inner: Arc<Mutex<Option<SendWrapper<T>>>>,
}
...
impl<T: 'static> BrowserMainType<T> {
    ...
    pub fn new(t: T) -> Self {
        Self {
            inner: Arc::new(Mutex::new(Some(SendWrapper::new(t)))),
        }
    }

    /// Tries to get the inner T
    pub fn with_mut(&self, f: impl FnOnce(&mut Option<SendWrapper<T>>)) {
